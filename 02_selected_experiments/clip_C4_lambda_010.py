...
run it.  Dependencies, input discovery, training, evaluation, saving and ZIP
packaging are automatic.
"""
import argparse
import csv
import hashlib
...
def train(
    ...
    for epoch in epoch_sequence:
        ...
        if epoch % args.sample_every == 0 or epoch == args.epochs:
            ...
            del preview
        # C4 is the final fixed-budget CLIP arm. Long-run continuation is out of
        # scope, so no 142 MB full training checkpoint is written.

    # Keep only the deployable/sampling weight. Raw G, D, optimizer and EMA
    # tracker states are intentionally omitted to minimize the final archive.
...
def main(argv=None):
    ...
    if args.zip_output:
        essential_names = [
            ...
            "training_log.csv",
            f"config_target_epoch_{args.epochs:03d}.json",
            f"samples_epoch_{args.epochs:03d}_final.png",
            f"metrics_epoch_{args.epochs:03d}.json",
            f"run_summary_epoch_{args.epochs:03d}.json",
        ]
        archive_path = str(
...
if __name__ == "__main__":
    print("[standalone] C4: final 50-epoch CLIP-MMD boundary arm with lambda=0.10.")
    main([
        "--lambda-clip", "0.10",
        "--clip-model", "ViT-B-32",
        "--epochs", "50",
