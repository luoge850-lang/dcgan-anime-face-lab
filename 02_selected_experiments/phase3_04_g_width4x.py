"""
================================================================================
 G强化实验 04_G_Width4x: 通道×4 — 容量边际收益终局测试
================================================================================
 PROBLEM: Width×2(FID=78.75)和Width×3(FID=59.0)证明容量方向有效。
          ×4是否还有边际收益？还是10M已达5层ConvTranspose架构的容量天花板？

 VARIABLE: 通道 ×4 (1024→512→256→128, ~18M params, G:D≈15:1)
          架构 = Baseline (5 ConvTranspose+BN+ReLU, bit-for-bit)
          NO Skip, NO extra depth, NO architecture change.

 RISK: G:D=15:1 远超文献建议的5:1上限
       可能反向失衡(G压倒D) → D_loss快速下降, D_real异常
       可能OOM (18M params在T4 16GB上可能内存紧张)

 PREDICTION: FID 50~60
   最优: FID < 55 → 容量方向仍有空间
   中性: FID ≈ 58 → 边际递减, ×3已达最优
   最差: FID > 60 或崩溃 → ×3是天花板
================================================================================
"""
...
# =============================================================================
# [04 CHANGE] Generator Width ×4 — 所有中间层通道翻四倍
#   1024→512→256→128 (vs Baseline 256→128→64→32, vs ×2 512→256→128→64, vs ×3 768→384→192→96)
#   Architecture = 00/01/03: 5 ConvTranspose+BN+ReLU. NO Skip. NO extra depth.
# =============================================================================
class Generator(nn.Module):
    def __init__(self, nd=NOISE_DIM):
        super().__init__()
        self.net=nn.Sequential(
            nn.ConvTranspose2d(nd,1024,4),nn.BatchNorm2d(1024),nn.ReLU(),
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: Width×4 (1024→512→256→128, ~18M params)")
    print(f"  G: {gp:,} params (5 ConvT) | D: {dp:,} params (Baseline)")
    print(f"  G:D = {gp/dp:.1f}:1 | Same arch as 00/01/03, only channels wider.")
    print(f"{'='*55}\n")
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":DATASET_LIMIT,
        "technique":"G Width×4: 1024→512→256→128, ~18M params (15:1 G:D). 5 ConvT+BN+ReLU, no Skip. Terminal capacity test — ×4 is the last width experiment.",
