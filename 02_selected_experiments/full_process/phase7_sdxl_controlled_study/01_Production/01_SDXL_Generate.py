"""
SDXL 候选图片生成 — Colab 粘贴即跑，零手动上传。

⚠ 只有需要更多 SDXL 图片时才用这个脚本。
   如果你已经有 1600 张从 Kaggle 生产的候选图, 直接跳到 02_Cleaning。

使用方法:
  1. colab.research.google.com → New Notebook → T4 GPU
  2. 本文件全文粘贴到一个 cell → Ctrl+Enter
  3. 模型自动从 HuggingFace 下载 (~6GB), 图片自动生成

续传 (可选):
  把 all_candidates_cumulative.zip 拖入 Colab 左侧文件浏览器 → /content/
  脚本自动检测并从上次停止处继续。

下载结果:
  Colab 左侧文件浏览器 → 右键 all_candidates_cumulative.zip → Download

输出:
  all_candidates_cumulative.zip — 包含 256px 预览、64px 训练图、1024px 原图、contact sheets
"""
...
# 🔧 内存碎片化防御：允许 PyTorch 使用可扩展段以避免 OOM
import os as _os
...
def _ensure_cuda() -> None:
    ...
    # 不要仅因为基准测试慢就重新安装 —— Colab 共享 GPU 可能导致首次运行较慢。
    # 重新安装会与其他库（diffusers 等）发生 TORCH_LIBRARY 命名空间冲突。
    try:
...
# 🔧 续传起始索引: 必须设为 Kaggle 已完成的数量，避免生成重复图片
# Kaggle 1600 张 = 设为 1600。如果你在 Kaggle 生成了其他数量，修改这个值。
# 0 仅用于：从 resume zip 恢复; 或你确认没有任何已有的 SDXL 候选图。
...
# 续传: 把 all_candidates_cumulative.zip 拖入 Colab 左侧文件浏览器即可
...
def determine_start_idx() -> int:
    if FORCE_START_INDEX > 0:
        print(f"[setup] FORCE_START_INDEX={FORCE_START_INDEX} — 跳过自动检测, 从索引 {FORCE_START_INDEX} 开始")
        print(f"[setup] sample_id 从 S{FORCE_START_INDEX+1:05d} 开始, seed 从 {SEED_BASE + FORCE_START_INDEX} 开始")
...
def main() -> None:
    ...
    print(f"  ─────────────────────────────────────────────")
    print(f"  💾 下载: Colab 左侧文件浏览器 → 右键 zip → Download")
    print(f"{'='*60}\n")
    ...
    print(f"  ─────────────────────────────────────────────")
    print(f"  💾 下载方法:")
    print(f"     Colab 左侧 → 📁 文件浏览器")
    print(f"     → 右键 all_candidates_cumulative.zip")
    print(f"     → Download")
    ...
    if start_idx + len(rows) < N_CANDIDATES:
        ...
        print(f"\n  下次 Colab 会话:")
        print(f"  1. 先把 all_candidates_cumulative.zip 下载到电脑")
        print(f"  2. 新 Colab → 拖 zip 到文件浏览器")
        print(f"  3. 粘贴本脚本 → 自动从 {next_idx} 续传")
    else:
        print(f"\n  🎉 ALL {N_CANDIDATES} DONE!")
        print(f"  下载 zip → 运行 03_Cleaning 清洗 → 消融训练")
