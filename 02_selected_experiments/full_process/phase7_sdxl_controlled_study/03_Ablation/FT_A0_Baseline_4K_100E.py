"""
FT-A0: Exp11 Fine-Tuning Baseline ? 100% ?? (4K), 100 Epoch
================================================================================
Kaggle GPU (T4?2), Internet ON (InceptionV3 ???)?
...
?? (/kaggle/working/dcgan_output/FT_A0_4K_100E/):
  - generator_ema_final.pth     ? ?? + ?????
  - discriminator_final.pth
  - baseline_reference.json     ? A20/A50 ???? (FID + Coverage + ????)
  - metrics.json, loss.csv, loss_curves.png, epoch_*.png
  - checkpoint_latest.pth       ? ????

??:
  ?? Exp11 ? G+D ?? ? ??? 4K ??? fine-tune 100 epoch
  DiffAugment + EMA ??????? 4K/100E ???? A0 ?????
"""
...
# ???????????????????????????????????????????????????????????????
# Coverage (k-NN Recall ? Kynk??nniemi et al. NeurIPS 2019)
# ???????????????????????????????????????????????????????????????
from scipy.spatial import cKDTree

def compute_coverage(rf: np.ndarray, ff: np.ndarray, k: int = 5) -> dict:
    """
    k-NN Recall (Coverage): ??????????????"??"?
    ...
    dist_real, _ = tree_real.query(rf, k=k + 1)  # +1 ????
    radii = dist_real[:, -1]  # ? k ??????

    tree_fake = cKDTree(ff)
    dist_fake, _ = tree_fake.query(rf, k=1)  # ??????
...
def main():
    ...
    all_paths = orig_paths  # A0: 100% ??, ? SDXL
    ...
    # ?? baseline_reference.json (? A20/A50 ????????) ??
    baseline_reference = {
        ...
        # ???? (A20/A50 ????)
        "coverage_abort_threshold": round(cov.get("coverage", 0) * 0.90, 4),   # < 90% ? ??
        "coverage_warn_threshold": round(cov.get("coverage", 0) * 0.95, 4),    # < 95% ? ??
        "fid_improvement_min": 0.5,          # FID ?? ? 0.5 ????? best
