"""
FT-A50: Exp11 Fine-Tuning — 50% 原图 + 50% SDXL (2K+2K), 100 Epoch
================================================================================
Kaggle GPU (T4×2), Internet ON。⚠ 需要 ~2,000 张清洗后的 SDXL 图片。

前提:
  - Kaggle Input 已上传 Exp11 权重 (generator_ema_final.pth, discriminator_final.pth)
  - Kaggle Input 已上传清洗后的 SDXL 图片 (≥2,000 张, accepted_manifest.csv + accepted_64/)
...
输出 (/kaggle/working/dcgan_output/FT_A50_4K_100E/):
  - generator_ema_final.pth, discriminator_final.pth
  - metrics.json, loss.csv, loss_curves.png, epoch_*.png
  - checkpoint_latest.pth

设计:
  加载 Exp11 的 G+D → 在 2000原图+2000SDXL 上 fine-tune 100 epoch
...
# 配置
# ═══════════════════════════════════════════════════════════════
EXPERIMENT_NAME = "FT_A50_4K_100E"
OUTPUT_DIR      = "/kaggle/working/dcgan_output"
EXP_DIR         = os.path.join(OUTPUT_DIR, EXPERIMENT_NAME)

N_ORIG  = 2000    # 原图数量
N_SDXL  = 2000    # SDXL 新图数量 (⚠ 需要 ≥2,000 张清洗后图片)
...
# ═══════════════════════════════════════════════════════════════
# 工具函数 (与 FT_A0/FT_A10/FT_A20 完全一致)
# ═══════════════════════════════════════════════════════════════
...
def compute_coverage(rf: np.ndarray, ff: np.ndarray, k: int = 5) -> dict:
    ...
    dist_fake, _ = tree_fake.query(rf, k=1); covered = (dist_fake.ravel() <= radii)
...
def locate_dataset() -> str:
    candidates = [
        ...
        "/kaggle/input/anime-faces/data", "/kaggle/input/anime_faces/data",
    ]
    for c in candidates:
        ...
        for d in dirs:
            if "anime" in d.lower() and "face" in d.lower():
                ...
                for sdp, sdirs, _ in os.walk(sub):
                    if "data" in sdirs: return os.path.join(sdp, "data")
...
def locate_exp11_weights() -> tuple[str, str]:
    ...
    for dp, _, fn in os.walk("/kaggle/input"):
        for f in fn:
            if "generator_ema_final" in f and f.endswith(".pth"): g_path = os.path.join(dp, f)
            if "discriminator_final" in f and f.endswith(".pth"): d_path = os.path.join(dp, f)
...
def locate_sdxl_images() -> list:
    for dp, _, fn in os.walk("/kaggle/input"):
        for f in fn:
            if f == "accepted_manifest.csv":
                manifest_path = os.path.join(dp, f)
                sdxl_dir = os.path.dirname(manifest_path)
                accepted_dir = os.path.join(sdxl_dir, "accepted_64")
                if os.path.isdir(accepted_dir):
                    paths = find_all_images(accepted_dir)
                    if paths:
                ...
                with open(manifest_path, newline="", encoding="utf-8-sig") as mf:
                    for row in csv.DictReader(mf):
                        p = row.get("path", "")
                        full = os.path.join(sdxl_dir, p) if not os.path.isabs(p) else p
    ...
    for dp, dirs, _ in os.walk("/kaggle/input"):
        for d in dirs:
            if d == "accepted_64":
                paths = find_all_images(os.path.join(dp, d))
                if paths:
                    print(f"[data] SDXL: {len(paths)} images from {os.path.join(dp, d)}")
                    return sorted(paths)
    # fallback: 搜含 "sdxl"/"clean" 关键词的目录或 zip
    for dp, dirs, _ in os.walk("/kaggle/input"):
        for d in dirs:
            dl = d.lower()
            if "sdxl" in dl or "clean" in dl:
                paths = find_all_images(os.path.join(dp, d))
                if paths:
                    print(f"[data] SDXL: {len(paths)} images from {os.path.join(dp, d)}")
                    return sorted(paths)
    for dp, _, fn in os.walk("/kaggle/input"):
        for f in fn:
            fl = f.lower()
            if f.endswith(".zip") and ("sdxl" in fl or "clean" in fl):
                extract_root = os.path.join("/kaggle/working/_sdxl_extracted", Path(f).stem)
                os.makedirs(extract_root, exist_ok=True)
                with zipfile.ZipFile(os.path.join(dp, f)) as z:
                    z.extractall(extract_root)
                paths = find_all_images(extract_root)
                if paths:
                    print(f"[data] SDXL: {len(paths)} images from zip {f}")
                    return sorted(paths)
    raise FileNotFoundError("Cannot locate SDXL cleaned images in /kaggle/input/ — "
                           "folder/zip name should contain 'sdxl' or 'clean'.")
        ...
        for f in fn:
            if f == "baseline_reference.json":
                ...
                with open(path) as jf:
                    ref = json.load(jf)
                print(f"[ref] Loaded baseline_reference: FID={ref.get('reference_FID','?')} "
...
def prepare_all_paths(dataset_dir: str) -> list:
    ...
    if len(sdxl_paths) < N_SDXL:
        raise RuntimeError(f"Need {N_SDXL} SDXL images but only found {len(sdxl_paths)}. "
                           f"Generate more candidates (need ~{int(N_SDXL/0.7)} total) or reduce N_SDXL.")
    set_all_seeds(SEED + 1000)
    sdxl_selected = sorted(random.sample(sdxl_paths, N_SDXL))
    print(f"[data] SDXL sampled: {len(sdxl_selected)}/{len(sdxl_paths)}")
    all_paths = orig_paths + sdxl_selected
...
def main():
    ...
    rf, rc = [], 0
    for imgs in eval_dl:
        rf.append(fid_calc._feat(imgs.to(DEVICE))); rc += imgs.size(0)
        if rc >= N_FID: break
    rf = np.concatenate(rf, axis=0)[:N_FID]
    ff, gc_ = [], 0
    with torch.no_grad():
        while gc_ < N_FID:
            bs = min(64, N_FID - gc_); noise = torch.randn(bs, NOISE_DIM, 1, 1, device=DEVICE)
            ff.append(fid_calc._feat(G(noise))); gc_ += bs
    ...
    G.load_state_dict(train_sd); del train_sd; del rf; del ff; del fid_calc
    ...
    metrics = {
        ...
        "Diversity": round(div, 4), "Laplacian_Variance": round(lap, 2),
        "Laplacian_Variance_Real": round(rlap, 2),
