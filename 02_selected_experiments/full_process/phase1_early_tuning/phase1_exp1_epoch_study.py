"""
================================================================================
 DCGAN Experiment 1: Epoch Study (50 / 100 / 200 / 300)
================================================================================
 Runs 4 epoch configurations sequentially in ONE Kaggle session.
 ...
 Output per epoch:
   ...
   exp1_epoch_300/
     ...
     epoch_050.png ...     ← 64-image fixed-noise grids
     real_images.png
     generator_final.pth
================================================================================
"""
...
DATASET_LIMIT = 8000    # Use only N images (set to None for all ~25k)
IMAGE_SIZE = 64; BATCH_SIZE = 32; NOISE_DIM = 100
LR = 1e-4; BETAS = (0.5, 0.99); SEED = 42; SAMPLE_INTERVAL = 50
...
def load_dataset():
    """Auto-discover GANAnime Lite dataset (25k anime faces). Returns (path, image_list)."""
    if os.path.exists(DATASET_PATH):
        imgs = find_all_images(DATASET_PATH)
        if imgs:
            print(f"Dataset: {DATASET_PATH} ({len(imgs)} images)")
            return DATASET_PATH, imgs
    ...
    for sub in sorted(os.listdir("/kaggle/input")):
        ...
        if os.path.isdir(sp):
            ...
            if imgs:
                print(f"Dataset: {sp} ({len(imgs)} images)")
                return sp, imgs
...
class FIDCalculator:
    ...
    def compute_fid(self, G, real_loader, n=10000):
        ...
        mr,sr=np.mean(rf,axis=0),np.cov(rf,rowvar=False)
        mf,sf=np.mean(ff,axis=0),np.cov(ff,rowvar=False)
...
def compute_diversity(G, lpips_calc, ns=500):
    ...
    imgs=torch.cat(imgs,dim=0)[:ns]
    npairs=2000; i1=torch.randint(0,ns,(npairs,)); i2=torch.randint(0,ns,(npairs,))
    scores=[]
...
def run_experiment(name, group, epochs, transform_fn, image_paths, fixed_noise):
    ...
    # DataLoader
    ds = AnimeDataset(image_paths, transform=transform_fn())
    ...
    eval_dl = DataLoader(eval_ds, batch_size=64, shuffle=True, drop_last=True, num_workers=2, pin_memory=True)

    print("FID ...")
    fid = FIDCalculator(DEVICE).compute_fid(G, eval_dl, n=10000)
    print(f"  FID: {fid:.2f}")
    gc.collect(); torch.cuda.empty_cache()

    print("LPIPS + Diversity + Laplacian ...")
    lpips_calc = LPIPSCalculator(DEVICE)
    rb,fb = [],[]
    generated = 0
    for imgs in eval_dl:
        rb.append((imgs.to(DEVICE)+1)/2.0)
        if sum(r.size(0) for r in rb)>=500: break
    ...
    metrics = {
        "experiment_name":name,"experiment_group":group,"epochs":epochs,"dataset_size":len(image_paths),
...
# =============================================================================
# Main — runs 4 epochs sequentially
# =============================================================================
def main():
    set_all_seeds(SEED)
    ...
    if DATASET_LIMIT and len(image_paths) > DATASET_LIMIT:
        set_all_seeds(SEED)
        image_paths = random.sample(image_paths, DATASET_LIMIT)
    ...
    for ep in [50, 100, 200, 300]:
        run_experiment(f"exp1_epoch_{ep:03d}", "exp1_epoch_study", ep, baseline_tf, image_paths, fixed_noise)
