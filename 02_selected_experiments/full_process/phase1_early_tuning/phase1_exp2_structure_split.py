"""
================================================================================
 DCGAN Experiment 2: Augmentation Ablation (Blur / Denoise / Sharpen — prob=0.2 each)
================================================================================
 Runs 3 structure augmentations split sequentially in ONE Kaggle session.
...
def main():
    ...
    def tf_blur(): return transforms.Compose([transforms.Resize((IMAGE_SIZE,IMAGE_SIZE)),transforms.RandomApply([transforms.GaussianBlur(3,(0.1,2.0))],p=0.2),transforms.ToTensor(),transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))])
    def tf_denoise(): return transforms.Compose([transforms.Resize((IMAGE_SIZE,IMAGE_SIZE)),MedianDenoise(prob=0.2),transforms.ToTensor(),transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))])
    def tf_sharpen(): return transforms.Compose([transforms.Resize((IMAGE_SIZE,IMAGE_SIZE)),EdgeSharpen(prob=0.2),transforms.ToTensor(),transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))])

    run_experiment("exp2_blur","exp2_structure_split",100,tf_blur,image_paths,fixed_noise)
    run_experiment("exp2_denoise","exp2_structure_split",100,tf_denoise,image_paths,fixed_noise)
    run_experiment("exp2_sharpen","exp2_structure_split",100,tf_sharpen,image_paths,fixed_noise)
