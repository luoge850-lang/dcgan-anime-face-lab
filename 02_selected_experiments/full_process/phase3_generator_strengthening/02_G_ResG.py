"""
================================================================================
 G强化实验 02_G_ResG: ConvTranspose + Skip Connection — 保留proven结构+加残差
================================================================================
 PROBLEM: 前三次尝试(Upsample+Conv+Skip/no-norm/IN)全部崩溃。
          根因: 完全替换了Baseline/Width×2中已证明稳定的ConvTranspose+BN+ReLU链。

 SOLUTION: 最小化改动——保留ConvTranspose+BN+ReLU作为主路径，
          只叠加Skip Connection(1×1Conv+Upsample)。

          不在G架构层面做任何其他改动:
          - 不替换ConvTranspose为Upsample+Conv
          - 不删除BatchNorm
          - 不引入InstanceNorm
          - 每个分辨率阶段只增加1条Skip旁路

 WHY this is the RIGHT approach:
   1. 01 (Width×2, FID=78.75) 证明 ConvTranspose+BN+ReLU 链对本次任务是稳定的
   2. 只加Skip → 改动最小化 → 收敛风险最低
   3. 如果Skip有效 → 后续再考虑用Upsample+Conv替换ConvTranspose
   4. 遵循"一次只改一个变量"的消融原则

 REFERENCE:
   SNGAN (Miyato et al., ICLR 2018): BN→ReLU→Upsample→Conv  pre-activation
   ResNet v2 (He et al., ECCV 2016): pre-activation > post-activation
   BigGAN (Brock et al., ICLR 2019): GBlock with skip from original x

 PREDICTION: FID 78~90
   如果 < 01(78.75) → Skip Connection 独立有效
   如果 ≈ 01 → Skip 在当前深度(5层)下不必要
   如果 > 90 → 即使最小改动，Skip 仍引入不稳定
================================================================================
"""
...
# =============================================================================
# [02 v4 CHANGE] Minimal ResBlock: 保留 ConvTranspose+BN+ReLU, 只加 Skip
#   主路径: ConvTranspose2d → BN → ReLU (与01/G_Width2x 逐bit一致)
#   Skip:   Upsample(bilinear) → Conv1×1 (独立的旁路, 匹配输出通道)
#   关键: 主路径 BIT-FOR-BIT = 01 (已验证稳定), Skip 是唯一新增变量
# =============================================================================
class ConvTResBlock(nn.Module):
    """Minimal residual block: keep proven ConvTranspose+BN+ReLU, add Skip only."""
    def __init__(self, in_ch, out_ch):
        super().__init__()
        # Main path: BIT-FOR-BIT identical to 00_baseline/01_Width×2 ConvTranspose stages
        self.main = nn.Sequential(
            nn.ConvTranspose2d(in_ch, out_ch, 4, 2, 1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU()
        )
        # Skip path: the ONLY new component — independent upsample+1×1
...
# =============================================================================
# [02 v4] Generator: Baseline structure + Skip at every upsampling stage
#   Architecture: noise(128) → ConvT(256@4×4) → ConvTResBlock×3 → final → RGB
#   Main path depth = Baseline (5 ConvT), same BN positions, same channels
#   Only addition: 3 Skip paths (1×1 Conv + Upsample per stage)
# =============================================================================
class Generator(nn.Module):
    def __init__(self, nd=NOISE_DIM):
        super().__init__()
        # Stage 0: noise → 4×4 (same as Baseline)
        self.init_proj = nn.Sequential(
            nn.ConvTranspose2d(nd, 256, 4),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
        # Stages 1-3: Baseline ConvTranspose + Skip Connection
        self.up1 = ConvTResBlock(256, 128)   # 4×4 → 8×8
        self.up2 = ConvTResBlock(128, 64)    # 8×8 → 16×16
        self.up3 = ConvTResBlock(64, 32)     # 16×16 → 32×32
        # Final: 32×32 → 64×64 RGB (same as Baseline)
    ...
    def forward(self, x):
        x = self.init_proj(x)
        x = self.up1(x)     # ConvT(256→128) + Skip
        x = self.up2(x)     # ConvT(128→64)  + Skip
        x = self.up3(x)     # ConvT(64→32)   + Skip
        x = self.final(x)
        return x

# =============================================================================
# Discriminator — Baseline D (SN+Hinge), UNCHANGED
# =============================================================================
...
# =============================================================================
# Metrics — FID, LPIPS, Diversity, Laplacian Variance, Edge Density
# =============================================================================
class FIDCalculator:
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: ConvTranspose+BN+ReLU + Skip Connection (v4 min-diff)")
    print(f"  G: {gp:,} params (5 ConvT + 3 Skip paths) | D: {dp:,} params (Baseline)")
    print(f"  Main path BIT-FOR-BIT = 00_baseline. Skip is the ONLY new component.")
    print(f"{'='*55}\n")
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":DATASET_LIMIT,
        "technique":"ResG v4 (min-diff): ConvTranspose+BN+ReLU (bit-for-bit = Baseline) + Skip Connection (Upsample+Conv1×1). Only new variable is the Skip path.",
