"""
================================================================================
 G强化实验 01_G_Width2x: Generator Width Expansion — 通道容量翻倍测试
================================================================================
 PROBLEM: Baseline G 的通道配置为 256→128→64→32 (≈1.1M params)。
          如果当前性能瓶颈来自 G 参数不足（不足以编码人脸分布），
          增加通道数是验证此假设的最直接方式。

 SOLUTION: 将 G 所有中间层通道数翻倍：512→256→128→64 (≈4.4M params)。
          D、Loss、训练策略完全不变。

 WHY Width first:
   1. 宽度是独立于深度的正交维度 — 先测宽度确定"容量瓶颈"，再测深度
   2. 2× Width = 4× 参数量，足以检测容量是否不足
   3. BigGAN (Brock et al., ICLR 2019) 证明 GAN 中 Width > Depth 的重要性
   4. 3× Width 会使 G:D 参数比超过 8:1 → 训练可能翻转(G压倒D)

 参考技术: BigGAN (Brock et al., ICLR 2019)
================================================================================
"""
...
# =============================================================================
# [01 CHANGE] Generator Width ×2 — 所有中间层通道翻倍
#   Baseline:  256 → 128 → 64  → 32  (≈1.1M params)
#   Width ×2:  512 → 256 → 128 → 64  (≈4.4M params, 4× capacity)
#   Architecture structure identical to Baseline, only channel counts doubled.
# =============================================================================
class Generator(nn.Module):
    def __init__(self, nd=NOISE_DIM):
        super().__init__()
        self.net=nn.Sequential(
            nn.ConvTranspose2d(nd,512,4),nn.BatchNorm2d(512),nn.ReLU(),
...
# =============================================================================
# Discriminator — Baseline D (SN+Hinge), UNCHANGED from 00_baseline
# =============================================================================
...
# =============================================================================
# Metrics — identical to 00_baseline (FID, LPIPS, Diversity, Laplacian Variance, Edge Density)
# =============================================================================
class FIDCalculator:
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: Baseline D + G Width×2 (512→256→128→64)")
    print(f"  G: {gp:,} params (4× Baseline capacity) | D: {dp:,} params (Baseline)")
    print(f"  Only change: Generator channel counts doubled. D and Loss unchanged.")
    print(f"{'='*55}\n")
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":DATASET_LIMIT,
        "technique":"G Width×2: Generator channels doubled (512→256→128→64, 4× params). D unchanged (SN+Hinge). Tests whether G capacity is the FID bottleneck.",
