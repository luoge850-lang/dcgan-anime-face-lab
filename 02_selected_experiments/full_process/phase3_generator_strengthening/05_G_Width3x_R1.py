"""
================================================================================
 G强化实验 05_G_Width3x_R1: Width×3 + R1 — 稳定化方向验证（无效）
================================================================================
 PROBLEM: Width×3的FID=59.0已到容量天花板, 但训练质量(梯度平滑度)
          仍有提升空间. R1梯度惩罚在深度调优阶段19_D_SN_Hinge_R1中
          已验证独立有效(FID -6.33), 与容量方向是正交维度.

 VARIABLE: R1 Gradient Penalty (gamma=1.0) — D Loss 加惩罚项
          G架构 = 03 Width×3 (bit-for-bit, 768→384→192→96, 5 ConvT)
          D架构 = Baseline (SN+Hinge, 不变)
          训练过程 = D_loss新增一项: gamma * 0.5 * ||∇D(real)||²

          唯一差异 vs 03: D_loss加了R1项
          对03的单变量对照: ✅ 严格 (G和D架构完全一致, 仅D_loss公式不同)

 IMPLEMENTATION:
   R1 = γ × ½ × ||∇_x D(x_real)||²
   real_img.detach().requires_grad_(True)  ← 只在计算R1时开梯度
   使用 torch.autograd.grad 而非 backward, 因为只需要对输入的梯度
   加到主 D_loss 上一同背传播

 REFERENCE:
   StyleGAN2 (Karras et al., CVPR 2020): R1 + R2 gradient penalty
   深度调优 19_D_SN_Hinge_R1: FID 96.25→89.92 (-6.33), γ=1.0
   gamma=1.0 已验证有效, 与深度调优19保持一致

 RESULT: FID=59.57 (△+0.57 vs 03). R1 完全无效.
   D_real 从 +0.32→−0.009, D 被过度约束.
   结论: Width×3 下 D 已在 Goldilocks, R1 是冗余约束.

 NOTE: D_step 计算量约翻倍 (autograd.grad 需要额外一次前向+反向)
================================================================================
"""
...
EXPERIMENT_NAME = "05_G_Width3x_R1"
...
EPOCHS=200; SAMPLE_INTERVAL=50; N_FID=10000; R1_GAMMA=1.0
...
# =============================================================================
# Generator — IDENTICAL to 03_G_Width3x (768→384→192→96, 5 ConvT+BN+ReLU)
# =============================================================================
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: Width×3 + R1 Gradient Penalty (γ={R1_GAMMA})")
    print(f"  G: {gp:,} params (=03) | D: {dp:,} params (=Baseline)")
    print(f"  Only change: D_loss += γ·½||∇D(real)||². G and D arch unchanged.")
    print(f"{'='*55}\n")
    ...
    for ep in range(1,EPOCHS+1):
        for img in dl:
            ...
            # === [06 CHANGE] Train D (Hinge + SN + R1) ===
            noise=torch.randn(bs,NOISE_DIM,1,1,device=DEVICE)
            with torch.no_grad(): fake=G(noise)
            d_fake=D(fake)

            # Main Hinge loss on real — requires grad on real for R1
            real.requires_grad_(True)
            d_real=D(real)
            d_loss_hinge=F.relu(1.0-d_real).mean()+F.relu(1.0+d_fake).mean()

            # R1 gradient penalty: γ × ½||∇D(real)||²
            r1_grads=torch.autograd.grad(outputs=d_real.sum(),inputs=real,
                                          create_graph=True,only_inputs=True)[0]
            r1_penalty=r1_grads.pow(2).sum(dim=[1,2,3]).mean()
            d_loss=d_loss_hinge+R1_GAMMA*0.5*r1_penalty
        ...
        dl_v=d_loss_hinge.item(); gl_v=g_loss.item()
        r1_penalty_val=r1_penalty.item()
        dr_v=d_real.mean().item(); df_v=d_fake.mean().item()
        w.writerow([ep,dl_v,gl_v,dr_v,df_v,r1_penalty_val])
        fdl,fgl,fdr,fdf=dl_v,gl_v,dr_v,df_v
        print(f"Epoch [{ep:3d}/{EPOCHS}]  D:{dl_v:.4f}  G:{gl_v:.4f}  DR:{dr_v:+.2f}  DF:{df_v:+.2f}  R1:{r1_penalty_val:.4f}")
    ...
    axes[1].plot(df["epoch"],df["D_loss"],color="#3498db",lw=1.5); axes[1].set(xlabel="Epoch",ylabel="Loss",title="D Loss (Hinge+R1)"); axes[1].grid(True,alpha=0.3)
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":DATASET_LIMIT,
        "technique":"Width×3 + R1 Gradient Penalty (γ=1.0). G/D architecture identical to 03. Only D_loss added: 0.5×||∇D(real)||².",
