"""
================================================================================
 #15: 13_D_SN_Hinge + SENet 1L — G/D dual-side optimal combo
================================================================================
 Single-variable experiment: vs 13_D_SN_Hinge (FID=96.25), only change is +1 SEBlock in G@L3.

 D side (from 13_D_SN_Hinge, FROZEN): SN(Conv) + Hinge, last Linear no SN
 G side (from 02_G_SENet_1layer, FROZEN): +1 SEBlock @ L3 (16×16, 64ch, r=16)

 Why this combo: 13_D_SN_Hinge proved D-side fix is #1 priority. 02_G_SENet_1layer proved SEBlock@L3 is
 best G-side module. They operate on independent dimensions (D loss function vs
 G channel attention) — should stack additively.

 If FID < 90: D+G dual optimization confirmed. Next = add Laplacian (λ=0.01).
 If FID ≈ 96: SEBlock benefit doesn't stack with Hinge — different mechanisms.
================================================================================
"""
...
EXPERIMENT_NAME = "15_D_SN_Hinge_G_SENet_1layer"
...
# =============================================================================
# SEBlock — from 02_G_SENet_1layer (unchanged)
# =============================================================================
...
# =============================================================================
# [#15 CHANGE] Generator = standard + 1 SEBlock @ L3 (16×16, 64ch)
#   This is the ONLY change vs 13_D_SN_Hinge. From 02_G_SENet_1layer.
# =============================================================================
class Generator(nn.Module):
    def __init__(self, nd=NOISE_DIM):
        ...
        self.se3 = SEBlock(64, reduction=16)          # ← ONLY new module
    ...
    def forward(self, x):
        x = self.up1(x)
        x = self.up2(x)
        x = self.se3(self.up3(x))    # L3: 64ch @ 16×16 — SE attention
        x = self.up4(x)
        x = self.up5(x)
        return x

# =============================================================================
# Discriminator — from 13_D_SN_Hinge (SN+Hinge, last Linear no SN), UNCHANGED
# =============================================================================
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: D3 (SN+Hinge) + SENet 1L @ G L3")
    print(f"  G: {gp:,} params (+1 SEBlock vs 13_D_SN_Hinge) | D: {dp:,} params (13_D_SN_Hinge, unchanged)")
    print(f"  Only change vs D3: +1 SEBlock(64, r=16) in Generator L3")
    print(f"{'='*55}\n")
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        "technique":"13_D_SN_Hinge(SN+Hinge)+SENet1L: Best D + Best G — single variable vs 13_D_SN_Hinge (+1 SEBlock@L3)",
