"""
================================================================================
 12_D_Hinge: Baseline + Hinge Loss — "Does fixing the loss help?"
================================================================================
 Change: Replace BCE Loss with Hinge Loss. D and G both standard architecture.

 Hinge Loss (Lim & Ye, 2017; Miyato et al., ICLR 2018):
   D_loss = ReLU(1 - D(real)) + ReLU(1 + D(fake))
   G_loss = -D(fake)

   BCE problem: when D is confident (D(fake)→0), BCE gradient → 0.
   Hinge fix: G gradient = -1 always, regardless of D confidence.
   D no longer outputs probability (no sigmoid) — just logits.

 Predicted FID: 100-108 (Baseline=109.40)
================================================================================
"""
...
# =============================================================================
# [12_D_Hinge CHANGE] Discriminator — standard, BUT no Sigmoid (Hinge needs logits)
# =============================================================================
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.net=nn.Sequential(
            ...
            nn.Linear(256,1))  # No Sigmoid — Hinge needs raw logits
    def forward(self,x): return self.net(x).view(-1)

# =============================================================================
# Metrics (standard — same as all experiments for comparability)
# =============================================================================
class FIDCalculator:
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: Hinge Loss (standard G/D)")
    print(f"  G: {gp:,} params | D: {dp:,} params | Device: {DEVICE}")
    print(f"  Loss: Hinge (no sigmoid on D) | Epochs: {EPOCHS}")
    print(f"{'='*55}\n")
    ...
    for ep in range(1,EPOCHS+1):
        for img in dl:
            ...
            # === [12_D_Hinge CHANGE] Train D with Hinge Loss ===
            ...
            # === [12_D_Hinge CHANGE] Train G with Hinge Loss ===
            noise=torch.randn(bs,NOISE_DIM,1,1,device=DEVICE)
            fake=G(noise)
            g_loss=-D(fake).mean()
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        "technique":"Hinge Loss (standard G/D, no SN)",
