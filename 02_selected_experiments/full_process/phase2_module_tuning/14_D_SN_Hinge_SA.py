"""
================================================================================
 14_D_SN_Hinge_SA: 13_D_SN_Hinge + Self-Attention — minimal diff from proven 13_D_SN_Hinge baseline
================================================================================
 v1 FAILED (FID=104.72): SA q/k/v had no SN → D(Real)=2.55 → Hinge dead.
 v2 FAILED (never run): SN added to q/k/v but stage2(64→128,s=1) still bloated
     D capacity +25% vs 13_D_SN_Hinge. SA placement masked by architecture drift.

 v3 FIX (TWO changes from v1):
   1. SA q/k/v now SN-constrained (from v2)
   2. REMOVED stage2(64→128,s=1) — SA now sits directly on stage1's 64ch@16×16
      Channel progression now IDENTICAL to 13_D_SN_Hinge: 3→32→64→[SA]→128→256→Lin
      13_D_SN_Hinge vs 14-v3: ONLY difference is SA presence. Clean ablation.

 Architecture: 3→32→64→[SA@64ch,16×16]→128→256→Flatten→Linear(4096→256)→Linear→1
 D params: ~1.28M (13_D_SN_Hinge=1.23M, +4% from SA's q/k/v 1×1 convs only)
================================================================================
"""
...
EXPERIMENT_NAME = "14_D_SN_Hinge_SA"
...
class AnimeDataset(Dataset):
    ...
    def __getitem__(self,i):
        for _ in range(10):  # Retry up to 10 random images
            try:
...
# =============================================================================
# Generator — standard model64.py, UNCHANGED
# =============================================================================
...
# =============================================================================
# [14_D_SN_Hinge_SA CHANGE] Self-Attention module
# =============================================================================
class SelfAttention(nn.Module):
    def __init__(self, in_ch):
        super().__init__()
        # [FIX] SN on q/k/v — without SN, SA bypasses all Lipschitz constraints
        # and D(Real) explodes to 2.55 (vs 13_D_SN_Hinge=0.02), saturating Hinge immediately.
        self.q=nn.utils.spectral_norm(nn.Conv2d(in_ch,in_ch//8,1))
        self.k=nn.utils.spectral_norm(nn.Conv2d(in_ch,in_ch//8,1))
        self.v=nn.utils.spectral_norm(nn.Conv2d(in_ch,in_ch,1))
        self.gamma=nn.Parameter(torch.zeros(1))
    def forward(self,x):
        B,C,H,W=x.size(); N=H*W
        q=self.q(x).view(B,-1,N); k=self.k(x).view(B,-1,N); v=self.v(x).view(B,-1,N)
        attn=F.softmax(torch.bmm(q.transpose(1,2),k),dim=-1)
        out=torch.bmm(v,attn.transpose(1,2)).view(B,C,H,W)
        return self.gamma*out+x

# =============================================================================
# [14_D_SN_Hinge_SA v3] Discriminator = 13_D_SN_Hinge + SA @ 16×16 (64ch)
#   Channel flow: 3→32→64→[SA@64ch]→128→256 — identical to 13_D_SN_Hinge, SA is the ONLY diff
# =============================================================================
class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        # Stage 1: 64→32→16 (same as 13_D_SN_Hinge layers 1-2)
        ...
        # Self-Attention @ 16×16, 64ch — 13_D_SN_Hinge has NO module here, this is the ONLY change
        self.attn=SelfAttention(64)
        # Stage 2: 16→8→4→1, 64→128→256 (identical to 13_D_SN_Hinge layers 3-4 + head)
        self.stage2=nn.Sequential(
            ...
            nn.Linear(256,1))  # No SN on last layer — D needs free output for Hinge ±1
    def forward(self,x):
        x=self.stage1(x)
        x=self.attn(x)
        x=self.stage2(x)
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME} [v3]: 13_D_SN_Hinge + SA — minimal diff from 13_D_SN_Hinge")
    print(f"  G: {gp:,} params (standard) | D: {dp:,} params (13_D_SN_Hinge + SA only)")
    print(f"  SA @ 16x16 (64ch) | Channel flow: 3→32→64→[SA]→128→256 | Device: {DEVICE}")
    print(f"{'='*55}\n")
    ...
    for ep in range(1,EPOCHS+1):
        for img in dl:
            ...
            g_loss=-D(G(torch.randn(bs,NOISE_DIM,1,1,device=DEVICE))).mean()
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        "technique":"13_D_SN_Hinge+SA: SN(Conv+qkv)+Hinge+SA@16x16(64ch) — D matched to 13_D_SN_Hinge channel flow, SA is only diff",
