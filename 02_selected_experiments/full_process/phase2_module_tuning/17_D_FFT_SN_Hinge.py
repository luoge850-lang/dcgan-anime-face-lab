"""
================================================================================
 #17: 13_D_SN_Hinge + FFT Frequency Branch in D — D-side frequency perception
================================================================================
 Single-variable experiment: vs 13_D_SN_Hinge (FID=96.25), ONLY change is
 adding a parallel FFT frequency-analysis branch to D's bottleneck.

 CRITICAL DESIGN: Additive fusion (spatial_logit + freq_logit), NOT concat.
   - spatial_head = Linear(256→1, no SN) — IDENTICAL to D3's last layer
   - fft_head = Linear(64→1, no SN) — tiny, ~65 params
   - logit = spatial_logit + freq_logit
   This ensures the spatial path is BIT-FOR-BIT identical to D3.
   Gradient flow from spatial_head to G is unchanged — FFT is a pure
   additive correction term that starts at ≈0 (default init).

 Why FFT (not Wavelet/DCT/Gabor):
   - DWT sub-bands ≈ what D's conv1 already learns (edges at multiple scales)
   - DCT is designed for compression, drops high-freq where GAN artifacts live
   - Gabor adds trainable params that can destabilize (new SA-style risk)
   - FFT gives D what 3×3 conv CANNOT see: periodic patterns (checkerboard,
     grid artifacts) and global frequency distribution — a genuinely new
     perceptual dimension orthogonal to spatial patch discrimination.

 Why parallel (not injected into middle layers):
   - Injecting into conv2 would change its input distribution → breaks SN's
     Lipschitz equilibrium (same failure mode as #14 SA and #16 SEBlock).
   - Parallel branch at bottleneck: main SN path intact, FFT is auxiliary.

 Why NO Spectral Norm on FFT branch:
   - Branch is ~23K params (1.9% of D's 1.23M) — too small to hijack D.
   - SN would suppress the already-tiny freq signal to near-zero → useless.
   - fft_head also no SN — scalar correction needs output freedom.

 Architecture:
   Main spatial path (IDENTICAL to 13_D_SN_Hinge):
     3→32→64→128→256 → Flatten → SN(Linear 4096→256) → LeakyReLU → 256-dim
     → Linear(256→1, no SN) → spatial_logit
   FFT branch (NEW, ~23K params):
     RGB→Gray → rfft2d → |·| → log1p → 3-layer mini-Conv(1→16→32→64ch) → 64-dim
     → Linear(64→1, no SN) → freq_logit
   Output:
     logit = spatial_logit + freq_logit  (additive, freq starts at ≈0)

 Decoupling principle:
   G dimension: global layout (standard, no explicit optimization)
   D dimension: local patches (3×3 conv) + frequency texture (FFT) ← NEW
   These are orthogonal perceptual dimensions → no gradient conflict expected.

 Prediction: FID 90-96. If FFT provides genuinely new information, it helps.
 If freq info is redundant with what conv layers already extract, mild degradation
 (like #16's +2.37). But it WILL NOT explode — branch is too small.
================================================================================
"""
...
EXPERIMENT_NAME = "17_D_FFT_SN_Hinge"
...
# =============================================================================
# Generator — standard (IDENTICAL to 13_D_SN_Hinge)
# =============================================================================
...
# =============================================================================
# [17_D_FFT_SN_Hinge CHANGE] Discriminator + parallel FFT frequency branch
# =============================================================================
class Discriminator(nn.Module):
    def __init__(self):
        ...
        # D3's original final layer — IDENTICAL structure, no SN
        # This is the EXACT same Linear(256→1) as 13_D_SN_Hinge
        self.spatial_head = nn.Linear(256, 1)

        # FFT frequency branch — parallel, lightweight, NO SN
        # Input: 64×64 → rfft2d → (64,33) magnitude → 3-layer mini-Conv → 64-dim
        self.fft_conv = nn.Sequential(
            nn.Conv2d(1, 16, 3, 2, 1), nn.LeakyReLU(0.2),   # 64×33 → 32×17
            nn.Conv2d(16, 32, 3, 2, 1), nn.LeakyReLU(0.2),  # 32×17 → 16×9
            nn.Conv2d(32, 64, 3, 2, 1), nn.LeakyReLU(0.2),  # 16×9 → 8×5
            nn.AdaptiveAvgPool2d(1),                          # → 1×1×64
            nn.Flatten(),                                      # → 64-dim
        )
        # Tiny head: frequency features → scalar correction (no SN, ~65 params)
        # Default init ≈ 0 → at epoch 0, D behaves almost exactly like D3
        self.fft_head = nn.Linear(64, 1)

    def _fft_branch(self, x):
        """Extract log-magnitude spectrum from grayscale input."""
        # x: (B, 3, H, W) in [-1, 1]
        # RGB → Gray using standard luminance weights
        gray = 0.299*x[:,0:1] + 0.587*x[:,1:2] + 0.114*x[:,2:3]
        # Real FFT → magnitude → log compression
        fft = torch.fft.rfft2(gray, norm='ortho')
        mag = torch.log1p(torch.abs(fft))
        return self.fft_conv(mag)

    def forward(self, x):
        feat = self.conv(x)                       # (B, 256) — spatial features
        spatial_logit = self.spatial_head(feat)   # (B, 1)   — identical to D3
        fft_feat = self._fft_branch(x)            # (B, 64)  — frequency features
        freq_logit = self.fft_head(fft_feat)      # (B, 1)   — freq correction (≈0 init)
        return (spatial_logit + freq_logit).view(-1)  # additive fusion

# =============================================================================
# Metrics (unchanged from 13_D_SN_Hinge)
# =============================================================================
class FIDCalculator:
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: D3 + FFT frequency branch in D")
    print(f"  G: {gp:,} params (standard, unchanged) | D: {dp:,} params (+FFT branch)")
    print(f"  Loss: Hinge | Device: {DEVICE} | Epochs: {EPOCHS}")
    print(f"  Single variable: +FFT frequency perception in D only")
    print(f"{'='*55}\n")
    ...
    axes[0].plot(df["epoch"],df["G_loss"],color="#e74c3c",lw=1.5); axes[0].set(title="G Loss (Hinge)"); axes[0].grid(True,alpha=0.3)
    axes[1].plot(df["epoch"],df["D_loss"],color="#3498db",lw=1.5); axes[1].set(title="D Loss (Hinge)"); axes[1].grid(True,alpha=0.3)
    ...
    axes[2].axhline(0.0,color="gray",ls="--",alpha=0.4); axes[2].legend(fontsize=8); axes[2].grid(True,alpha=0.3)
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        "technique":"13_D_SN_Hinge + FFT frequency branch in D: parallel rfft2d→magnitude→log1p→mini-Conv(1→16→32→64ch, ~23K params, NO SN). ADDITIVE fusion: spatial_head(256→1, identical to D3) + fft_head(64→1, ~65 params). logit = spatial_logit + freq_logit. Spatial path is bit-for-bit D3; freq starts at ≈0 (default init).",
