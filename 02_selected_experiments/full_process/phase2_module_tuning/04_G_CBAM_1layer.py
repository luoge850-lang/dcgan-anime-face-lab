"""
================================================================================
 Deep Tuning 04_G_CBAM_1layer: Generator + 1-Layer CBAM @ L3 | 200 Epochs
================================================================================
...
# =============================================================================
EXPERIMENT_NAME = "04_G_CBAM_1layer"
...
# =============================================================================
# [修改位置] Generator with 1-Layer CBAM — 仅L3(16×16,64ch)加CBAM Block
#
# 为什么仅L3:
#   L1(4×4): 7×7 conv覆盖100%特征图 — 空间注意力退化为全局偏置
#   L2(8×8): 7×7 conv覆盖77% — 近似全局，无"人脸vs背景"区分能力
#   L3(16×16): 7×7 conv覆盖19% — 恰好捕捉"眼睛区域"等有意义空间模式
#   L4(32×32): 7×7 conv仅覆盖5% — 感受野太局部，看不到全局结构
#
# 对比 4-Layer CBAM 的变化:
#   - 3 个 CBAMBlock 实例 (cbam1/cbam2/cbam4 已移除)
#   +1 个 CBAMBlock (cbam3) — 约1.1K参数
#   forward() 中仅 L3 多一次 self.cbam3(...) 调用
...
class Generator(nn.Module):
    def __init__(self, nd=NOISE_DIM):
        ...
        # Layer 1: 1x1 -> 4x4, 256 channels — NO CBAM (too small: 7x7 covers entire 4x4 map)
        self.up1 = nn.Sequential(
            nn.ConvTranspose2d(nd,256,4), nn.BatchNorm2d(256), nn.ReLU())

        # Layer 2: 4x4 -> 8x8, 128 channels — NO CBAM (7x7 covers 77% of 8x8)
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(256,128,4,2,1), nn.BatchNorm2d(128), nn.ReLU())

        # Layer 3: 8x8 -> 16x16, 64 channels — [1-LAYER CBAM] ONLY attention here
        # At 16x16, 7x7 kernel covers 19% of map — perfect for spatial attention
        self.up3 = nn.Sequential(
            nn.ConvTranspose2d(128,64,4,2,1), nn.BatchNorm2d(64), nn.ReLU())
        self.cbam3 = CBAMBlock(64)

        # Layer 4: 16x16 -> 32x32, 32 channels — NO CBAM
    ...
    def forward(self, x):
        x = self.up1(x)
        x = self.up2(x)
        x = self.cbam3(self.up3(x))  # [CBAM唯一插入点] L3: 64ch @ 16x16
        x = self.up4(x)
        x = self.up5(x)
        return x
...
def main():
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        ...
        "architecture":"Generator + 1-layer CBAM at L3 (64ch@16x16)",
