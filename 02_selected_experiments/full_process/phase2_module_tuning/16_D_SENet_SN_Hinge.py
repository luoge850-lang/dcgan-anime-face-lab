"""
================================================================================
 #16: 13_D_SN_Hinge + SEBlock in D — Channel attention for the Discriminator
================================================================================
 Single-variable: vs 13_D_SN_Hinge (FID=96.25), only change is +1 SEBlock
 in D at 16×16, 64ch (after conv1, before conv2).

 SEBlock placement in D:
   D: 3→32(s=2)→64(s=2)→[SEBlock@16×16,64ch]→128(s=2)→256(s=2)→Flatten→Lin
   G: nd→256→128→64→[SEBlock@16×16,64ch]→32→3   (from 02_G_SENet_1layer)
   Both place SEBlock at 16×16, 64ch — the feature sweet spot.

 Why D-side SEBlock: D's 3×3 convolutions only see local patches. Channel
 attention lets D learn which feature channels matter most for real-vs-fake
 discrimination, without adding spatial attention (SA failed 3× in 14_D_SN_Hinge_SA).

 Prediction: FID 90-96. Less impactful than G-side SEBlock because D's job
 is criticism, not generation — channel attention helps G's generation more
 than D's discrimination. But should be stable (unlike SA).
================================================================================
"""
...
EXPERIMENT_NAME = "16_D_SENet_SN_Hinge"
...
# =============================================================================
# Generator — standard model64.py, UNCHANGED from 13_D_SN_Hinge
# =============================================================================
...
# =============================================================================
# SEBlock — identical to 02_G_SENet_1layer
# =============================================================================
...
# =============================================================================
# [#16 CHANGE] Discriminator = 13_D_SN_Hinge + SEBlock @ 16×16, 64ch
#   Channel flow: 3→32→64→[SEBlock]→128→256 — identical to 13_D_SN_Hinge, +1 SEBlock
# =============================================================================
class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        # Conv1: 64→32→16, 3→64ch
        ...
        # SEBlock @ 16×16, 64ch — D's channel attention (ONLY new module)
        self.se = SEBlock(64, reduction=16)
        # Conv2: 16→8→4→1, 64→128→256→Lin  (identical to 13_D_SN_Hinge)
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.se(x)      # Channel attention @ 16×16, 64ch
        x = self.conv2(x)
        return x.view(-1)
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: D3 + SEBlock in D @ 16×16, 64ch")
    print(f"  G: {gp:,} params (standard) | D: {dp:,} params (+1 SEBlock vs 13_D_SN_Hinge)")
    print(f"  Only change vs 13_D_SN_Hinge: +1 SEBlock(64, r=16) in D after conv1")
    print(f"{'='*55}\n")
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        "technique":"13_D_SN_Hinge+SEBlock(D): SN(Conv)+Hinge+SEBlock@16×16(64ch) in D — channel attention for D",
