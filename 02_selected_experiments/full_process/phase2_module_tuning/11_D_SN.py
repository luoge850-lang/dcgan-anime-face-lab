"""
================================================================================
 11_D_SN: Baseline + Spectral Normalization — "Does constraining D help?"
================================================================================
 Change: Add Spectral Normalization to ALL Conv/Linear layers in Discriminator.
 Everything else = Baseline (standard G, BCE loss, lr=1e-4, batch=32, 200ep).

 Spectral Norm (Miyato et al., ICLR 2018):
   Constrains ||W||_2 = 1 for every layer → Lipschitz-1 D.
   Without SN, D can develop arbitrarily steep decision boundaries around real
   data points → D overfits → G receives vanishing gradients.
   SN forces D to be smooth → G always gets meaningful gradients.

 Predicted FID: 95-105 (Baseline=109.40)
================================================================================
"""
import torch.nn.utils as sn_utils  # noqa: F401 — used via nn.utils.spectral_norm
# (Script is structurally identical to 00_Baseline with one diff: D layers wrapped in spectral_norm)
...
class AnimeDataset(Dataset):
    ...
    def __getitem__(self,i): img=Image.open(self.paths[i]).convert("RGB"); return self.tf(img) if self.tf else img
...
# =============================================================================
# [11_D_SN CHANGE] Discriminator + Spectral Normalization (optimized: Conv+penLin only)
#   SN on 4 Conv + penLin(4096→256) — lastLin(256→1) FREE (avoids double-constraint with Sigmoid)
#   Updated 2026-07-22: SN placement optimized based on 13_D_SN_Hinge findings
# =============================================================================
...
def main():
    ...
    del g_tmp,d_tmp
    print(f"\n{'='*55}")
    print(f"  {EXPERIMENT_NAME}: SN(Conv+penLin, lastLin free) + BCE")
    print(f"  G: {gp:,} params (standard) | D: {dp:,} params (+SN, lastLin free)")
    print(f"  Loss: BCE (unchanged) | Device: {DEVICE} | Epochs: {EPOCHS}")
    print(f"{'='*55}\n")
    ...
    axes[0].plot(df["epoch"],df["G_loss"],color="#e74c3c",lw=1.5); axes[0].set(title="G Loss"); axes[0].grid(True,alpha=0.3)
    axes[1].plot(df["epoch"],df["D_loss"],color="#3498db",lw=1.5); axes[1].set(title="D Loss"); axes[1].grid(True,alpha=0.3)
    ...
    metrics={"experiment_name":EXPERIMENT_NAME,"epochs":EPOCHS,"dataset_size":len(image_paths),
        "technique":"Spectral Normalization in D (optimized: Conv+penLin only, lastLin free + Sigmoid) + BCE loss. Standard G. SN placement matched to 13_D_SN_Hinge findings.",
    ...
    print(f"\n  Complete: {EXPERIMENT_NAME}  FID={fid:.2f}  LPIPS={lpips_mean:.4f}  Div={div:.4f}")
