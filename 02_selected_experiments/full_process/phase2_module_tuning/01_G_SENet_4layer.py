"""
================================================================================
 Deep Tuning 01_G_SENet_4layer: Generator + 4-Layer SENet | 200 Epochs
================================================================================
...
# =============================================================================
EXPERIMENT_NAME = "01_G_SENet_4layer"
...
# =============================================================================
# =============================================================================
# [修改位置] Generator with SENet — 唯一改动：在中间4层加SEBlock
#
...
#
# 修改后 Generator (SENet):
#   up1 -> [SE1] -> up2 -> [SE2] -> up3 -> [SE3] -> up4 -> [SE4] -> up5
#   ↑ 在中间 4 层的 BN+ReLU 后插入 SEBlock
#   ↑ 最后一层 (up5, 32→3, Tanh) 不加 SE——输出层是RGB像素，无"通道注意力"
#
# 对比 Baseline 的变化:
#   +4 个 SEBlock 实例 (se1~se4)
#   +约 10.9K 参数 (< Generator 总参数的 1%)
#   forward() 中每层多了 self.seN(...) 调用
...
class Generator(nn.Module):
    def __init__(self, nd=NOISE_DIM):
        ...
        # 原始: ConvTranspose2d(100,256,4)
        # 这是从噪声向量到第一个空间特征图的关键层
        self.up1 = nn.Sequential(
            nn.ConvTranspose2d(nd,256,4), nn.BatchNorm2d(256), nn.ReLU())
        # [SENet 插入点 #1] 256 channels, reduction=16 -> bottleneck=16
        self.se1 = SEBlock(256, reduction=16)

        # ===== Layer 2: 4x4 -> 8x8, 128 channels =====
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(256,128,4,2,1), nn.BatchNorm2d(128), nn.ReLU())
        # [SENet 插入点 #2] 128 channels, reduction=16 -> bottleneck=8
        self.se2 = SEBlock(128, reduction=16)

        # ===== Layer 3: 8x8 -> 16x16, 64 channels =====
        self.up3 = nn.Sequential(
            nn.ConvTranspose2d(128,64,4,2,1), nn.BatchNorm2d(64), nn.ReLU())
        # [SENet 插入点 #3] 64 channels, reduction=16 -> bottleneck=4
        self.se3 = SEBlock(64, reduction=16)

        # ===== Layer 4: 16x16 -> 32x32, 32 channels =====
        self.up4 = nn.Sequential(
            nn.ConvTranspose2d(64,32,4,2,1), nn.BatchNorm2d(32), nn.ReLU())
        # [SENet 插入点 #4] 32 channels, reduction=16 -> bottleneck=2
        self.se4 = SEBlock(32, reduction=16)

        # ===== Layer 5: 32x32 -> 64x64, 3 channels (RGB output) =====
        # 不加 SENet —— 输出层是 RGB 像素，通道间是(R,G,B)，
        # 它们之间不存在"哪个通道更重要"的问题
        self.up5 = nn.Sequential(
            nn.ConvTranspose2d(32,3,4,2,1), nn.Tanh())

    def forward(self, x):
        # 数据流: x(100) -> up1 -> se1(通道重标定) -> up2 -> se2 -> ...
        # 每一步 SEBlock 在空间特征图进入下一个上采样层之前，
        # 对当前层的通道做一次重要性筛选
        x = self.se1(self.up1(x))
        x = self.se2(self.up2(x))
        x = self.se3(self.up3(x))
        x = self.se4(self.up4(x))
        x = self.up5(x)   # 最终输出 RGB，无 SE
        return x
...
def main():
    ...
    print(f"  Architecture: Generator + SE Blocks (reduction=16)")
    ...
    metrics={
        ...
        "augmentation":"Flip(p=0.5)+EdgeSharpen(p=0.2)",
        "architecture":"Generator + SE Blocks (reduction=16)",
